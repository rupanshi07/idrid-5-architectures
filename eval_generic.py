...
def get_args():
    ...
    p.add_argument("--model_module", required=True)
...
def main():
    ...
    try:
        model = JointSegClsNet(num_lesion_classes=len(LESIONS), pretrained=False).to(device)
    except TypeError:
    ...
    if all_preds:
        ...
        print("\nConfusion matrix:")
        print(cm)
        plot_confusion_matrix(cm, [str(i) for i in range(5)], f"checkpoints_{args.tag}/confusion_matrix.png",
                               f"DR Grading -- {args.tag}")
