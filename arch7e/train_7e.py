...
from model_7e import JointSegClsNet
...
def get_args():
    ...
    p.add_argument("--out_dir", default="checkpoints_7e")
    return p.parse_args()
...
def main():
    ...
    print(f"[7E] Using device: {device}")
    ...
    # 4 parameter groups now, per faculty's specific recommendation:
    # encoder 1e-5, bottleneck transformer 5e-5, decoder 1e-4, classifier 2e-4
    ...
    for epoch in range(args.epochs):
        ...
        model.train()
        running = 0.0
        pbar = tqdm(train_loader, desc=f"[7E] Epoch {epoch+1}/{args.epochs}")
        for batch in pbar:
            image = batch["image"].to(device); mask = batch["mask"].to(device)
    ...
    print(f"\n[7E] Training complete. Best combined score: {best_score:.4f}")
