...
def get_args():
    ...
    p.add_argument("--checkpoint", default="checkpoints_7a_ordinal/model_best.pth")
    p.add_argument("--cm_out", default="checkpoints_7a_ordinal/confusion_matrix.png")
    return p.parse_args()
...
def main():
    ...
    print(f"[Arch 7A Ordinal] Using device: {device}")
    ...
    model = JointSegClsNet(num_lesion_classes=len(LESIONS), num_dr_classes=4, pretrained=False).to(device)
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for i in range(image.size(0)):
                ...
                if cls_valid[i] == 1:
                    grade_pred = ordinal_to_grade(cls_pred[i:i+1]).item()
    ...
    if all_preds:
        ...
        print("\nConfusion matrix:")
        print(cm)
        plot_confusion_matrix(cm, [str(i) for i in range(5)], args.cm_out, "DR Grading -- Arch 7A Ordinal (EX-guided)")
