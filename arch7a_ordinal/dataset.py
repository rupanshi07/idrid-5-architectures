...
LESIONS = ["EX"]
LESION_FOLDER_MAP = {"EX": "Hard_Exudates"}
...
class IDRiDJointDataset(Dataset):
    def __init__(self, seg_img_dir=None, seg_mask_dir=None, cls_img_dir=None, cls_csv=None,
                 image_size=512, augment=False, zoom_crop_prob=0.35, use_clahe=True):
        ...
        self.color_jitter = T.ColorJitter(brightness=0.15, contrast=0.15, saturation=0.15, hue=0.02)
    ...
    def class_counts(self, num_classes=5):
        ...
        for s in self.samples:
            if s["has_cls"] and 0 <= s["dr_grade"] < num_classes:
                counts[s["dr_grade"]] += 1
        return counts
    ...
    def _zoom_crop_pair(self, image, raw_masks, scale_range=(0.70, 1.0)):
        if random.random() >= self.zoom_crop_prob:
            return image, raw_masks
        w, h = image.size
        ...
        for k, v in raw_masks.items():
            new_masks[k] = v.crop((left, top, right, bottom)) if v is not None else None
        image = image.resize((self.image_size, self.image_size), Image.BILINEAR)
        for k, v in new_masks.items():
            if v is not None:
                new_masks[k] = v.resize((self.image_size, self.image_size), Image.NEAREST)
...
class IDRiDSubset(Dataset):
    ...
    def sample_weights(self):
        ...
        return [0.5 / max(n_seg, 1) if hs else 0.5 / max(n_cls, 1) for hs in has_seg_list]

    def compute_pos_weight(self, num_lesions, image_size, max_weight=50.0):
        total_pos = torch.zeros(num_lesions, dtype=torch.float32)
        total_pixels = 0
