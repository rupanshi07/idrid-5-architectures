...
def get_args():
    ...
    p.add_argument("--checkpoint", default="checkpoints_5_gan/model_best.pth")
    p.add_argument("--cm_out", default="checkpoints_5_gan/confusion_matrix.png")
    return p.parse_args()
...
def main():
    ...
    # Only the Generator is needed for inference -- the Discriminator was
    # a training-time-only auxiliary network, not part of the final model.
    model = Generator(num_lesion_classes=len(LESIONS), pretrained=False).to(device)
    ...
    if all_preds:
        ...
        print("\nConfusion matrix:")
        print(cm)
        plot_confusion_matrix(cm, [str(i) for i in range(5)], args.cm_out, "DR Grading -- Architecture 5: GAN")
