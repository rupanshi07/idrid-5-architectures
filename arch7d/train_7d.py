...
from model_7d import JointSegClsNet
...
def compute_loss(seg_out, mask, seg_valid, cls_out, dr_grade, cls_valid, seg_focal_alpha, seg_weight, cls_weight):
    if seg_valid.sum() > 0:
...
def get_args():
    ...
    p.add_argument("--out_dir", default="checkpoints_7d")
    return p.parse_args()
...
def main():
    ...
    print(f"[7D] Using device: {device}")
    ...
    for name, param in model.named_parameters():
        ...
        elif name.startswith("cls_head.") or name.startswith("bottleneck_transformer."):
    ...
    for epoch in range(args.epochs):
        ...
        model.train()
        running = 0.0
        pbar = tqdm(train_loader, desc=f"[7D] Epoch {epoch+1}/{args.epochs}")
        for batch in pbar:
            image = batch["image"].to(device); mask = batch["mask"].to(device)
            ...
            with torch.autocast(device_type="cuda", enabled=(device.type == "cuda")):
                seg_pred, cls_pred = model(image)
                loss, seg_l, cls_l = compute_loss(seg_pred, mask, seg_valid, cls_pred, dr_grade, cls_valid,
        ...
        print(f"Epoch {epoch+1}: val Dice={val_dice:.4f} | val Kappa={val_kappa:.4f} | combined(0.7K+0.3D)={combined:.4f}")
    ...
    print(f"\n[7D] Training complete. Best combined score: {best_score:.4f}")
