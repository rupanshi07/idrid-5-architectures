"""
ARCHITECTURE 7F -- Full faculty specification:
Pretrained Swin-Tiny -> multi-scale features -> E4 bottleneck ->
Bottleneck Transformer -> [feeds decoder for EX/SE segmentation] AND
[fused with EX/SE probability-weighted features + global features] ->
Classification Transformer -> Attention Pooling -> DR grade (5-class)
"""
import torch
...
class MaskGuidedFusion(nn.Module):
    """Fuses: global bottleneck features + EX-guided features + SE-guided
    features -- via SOFT (continuous probability) gating, never a hard
    0.5 threshold. Produces a spatial feature map (same shape as input)
    ready to be tokenized for the classification transformer."""
    def __init__(self, channels=768):
        ...
        self.ex_embed = nn.Sequential(
            ...
            nn.Conv2d(64, channels, kernel_size=1, bias=False), nn.BatchNorm2d(channels),
        )
    ...
    def forward(self, bottleneck_feat, ex_prob, se_prob):
        ex_prob = F.interpolate(ex_prob, size=bottleneck_feat.shape[-2:], mode="bilinear", align_corners=False)
        se_prob = F.interpolate(se_prob, size=bottleneck_feat.shape[-2:], mode="bilinear", align_corners=False)

        global_feat = self.global_proj(bottleneck_feat)
        ex_feat = self.ex_embed(ex_prob)
        se_feat = self.se_embed(se_prob)

        fused = global_feat + ex_feat + se_feat
        return fused


class ClassificationTransformerWithAttentionPooling(nn.Module):
    """A SEPARATE, dedicated Transformer for classification (distinct
    from the shared bottleneck transformer), followed by learned
    ATTENTION POOLING (not simple mean pooling) -- the model learns
    which tokens/regions matter most for DR grading."""
    def __init__(self, channels=768, num_layers=2, num_heads=8, mlp_ratio=4, dropout=0.1, num_classes=5):
        super().__init__()
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=channels, nhead=num_heads, dim_feedforward=channels * mlp_ratio,
            dropout=dropout, activation="gelu", batch_first=True, norm_first=True,
        ...
        self.classifier = nn.Sequential(
            ...
            nn.Linear(channels, 256), nn.GELU(), nn.Dropout(0.3),
            nn.Linear(256, num_classes),
        )

    def forward(self, fused_feat):
        B, C, H, W = fused_feat.shape
        tokens = fused_feat.flatten(2).transpose(1, 2)
        tokens = self.transformer(tokens)
...
class JointSegClsNet(nn.Module):
    def __init__(self, num_lesion_classes=2, num_dr_classes=5, pretrained=True):
        ...
        self.fusion = MaskGuidedFusion(channels=768)
        self.cls_head = ClassificationTransformerWithAttentionPooling(
            channels=768, num_layers=2, num_heads=8, num_classes=num_dr_classes,
        )
    ...
    def forward(self, x):
        ...
        # SOFT probability maps (never hard-thresholded) guide classification
        seg_prob = torch.sigmoid(seg_out)
        ex_prob = seg_prob[:, 0:1]
        se_prob = seg_prob[:, 1:2]

        fused = self.fusion(e4_context, ex_prob, se_prob)
