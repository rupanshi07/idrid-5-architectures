...
from model_7f import JointSegClsNet
from losses_7f import segmentation_loss, hybrid_classification_loss
...
def compute_loss(seg_out, aux_outs, mask, seg_valid, cls_out, dr_grade, cls_valid,
                  class_weights, seg_weight, cls_weight, aux_weights=(0.4, 0.2)):
    if seg_valid.sum() > 0:
        valid_idx = seg_valid.bool()
        seg_loss = segmentation_loss(seg_out[valid_idx], mask[valid_idx])
        for aux_out, w in zip(aux_outs, aux_weights):
            seg_loss = seg_loss + w * segmentation_loss(aux_out[valid_idx], mask[valid_idx])
    else:
    ...
    if cls_valid.sum() > 0:
        ...
        cls_loss = hybrid_classification_loss(cls_out[valid_idx], safe_target[valid_idx], class_weights=class_weights)
    else:
...
def get_args():
    ...
    p.add_argument("--out_dir", default="checkpoints_7f")
    return p.parse_args()
...
def main():
    ...
    print(f"[7F] Using device: {device}")
    ...
    print(f"Segmentation pos_weight: {pos_weight}")

    counts = full_dataset.class_counts()
    print(f"Classification counts: {counts}")
    inv_freq = [1.0 / c if c > 0 else 0.0 for c in counts]
    total_inv = sum(inv_freq)
    class_weights = torch.tensor([w / total_inv * 5 for w in inv_freq], dtype=torch.float32).to(device)
    ...
    for name, param in model.named_parameters():
        ...
        elif name.startswith("bottleneck_transformer."):
            bottleneck_params.append(param)
        elif name.startswith("cls_head.") or name.startswith("fusion."):
    ...
    for epoch in range(args.epochs):
        ...
        model.train()
        running = 0.0
        pbar = tqdm(train_loader, desc=f"[7F] Epoch {epoch+1}/{args.epochs}")
        for batch in pbar:
            image = batch["image"].to(device); mask = batch["mask"].to(device)
            ...
            with torch.autocast(device_type="cuda", enabled=(device.type == "cuda")):
                ...
                loss, seg_l, cls_l = compute_loss(seg_pred, aux_preds, mask, seg_valid, cls_pred, dr_grade, cls_valid,
                                                   class_weights, args.seg_weight, args.cls_weight)
        ...
        # Faculty's fixed weighting: Kappa > Dice > Accuracy
        combined = 0.50 * max(val_kappa, 0.0) + 0.30 * val_dice + 0.20 * val_acc
        print(f"Epoch {epoch+1}: val Dice={val_dice:.4f} | val Kappa={val_kappa:.4f} | val Acc={val_acc:.4f} | "
              f"combined(0.5K+0.3D+0.2A)={combined:.4f}")
    ...
    print(f"\n[7F] Training complete. Best combined score: {best_score:.4f}")
