...
from model_7a_v3 import JointSegClsNet
...
def compute_loss(seg_out, aux_outs, mask, seg_valid, cls_out, dr_grade, cls_valid,
    ...
    if seg_valid.sum() > 0:
        ...
        # Deep supervision: same Dice+Focal recipe at each auxiliary depth,
        # weighted lower than the main output (per faculty's recommendation)
...
def get_args():
    ...
    p.add_argument("--out_dir", default="checkpoints_7a_v3")
    return p.parse_args()
...
def main():
    ...
    print(f"[7A v3] Using device: {device}")
    ...
    # Freeze Swin encoder for the first `freeze_epochs` epochs, per feedback
    model.set_encoder_trainable(False)
    print(f"Swin encoder FROZEN for the first {args.freeze_epochs} epochs")

    encoder_params, decoder_params, classifier_params = [], [], []
    ...
    # Warm-up (linear) then cosine decay, per feedback
    def lr_lambda(epoch):
    ...
    for epoch in range(args.epochs):
        ...
        model.train()
        running = 0.0
        pbar = tqdm(train_loader, desc=f"[7A-v3] Epoch {epoch+1}/{args.epochs}")
        for batch in pbar:
            image = batch["image"].to(device); mask = batch["mask"].to(device)
        ...
        # Checkpoint metric: 70% Kappa + 30% Dice, per feedback
        combined = 0.7 * max(val_kappa, 0.0) + 0.3 * val_dice
        print(f"Epoch {epoch+1}: val Dice={val_dice:.4f} | val Kappa={val_kappa:.4f} | "
              f"combined(0.7K+0.3D)={combined:.4f} | LR={[g['lr'] for g in optimizer.param_groups]}")
    ...
    print(f"\n[7A v3] Training complete. Best combined score: {best_score:.4f}")
