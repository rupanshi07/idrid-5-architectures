"""
ARCHITECTURE 7A v3 -- per faculty feedback:
- Swin-T encoder (unchanged), 5-class classification (NOT ordinal)
- Attention-pooling classifier fusing multi-scale features (e1-e4)
- Deep supervision: auxiliary segmentation heads at 2 decoder depths
"""
import torch
...
class AttentionPoolingClassifier(nn.Module):
    """Fuses multi-scale features (e1, e2, e3, e4) instead of only the
    deepest layer. Each scale is pooled to a vector, projected to a
    common dimension, then combined via LEARNED ATTENTION WEIGHTS
    (not simple averaging) -- the network learns which scale matters
    most for grading, per faculty's recommendation."""
    def __init__(self, channels_list=(96, 192, 384, 768), common_dim=256, num_classes=5):
        ...
        # Attention scorer: learns one importance score per scale
    ...
    def forward(self, features):
        # features: list of [e1, e2, e3, e4] feature maps
        projected = [proj(f) for proj, f in zip(self.projections, features)]  # each (B, common_dim)
        stacked = torch.stack(projected, dim=1)  # (B, 4, common_dim)
        scores = self.attn_score(stacked)  # (B, 4, 1)
        weights = torch.softmax(scores, dim=1)  # attention weights across the 4 scales
        pooled = (stacked * weights).sum(dim=1)  # (B, common_dim) -- weighted fusion
        return self.classifier(pooled)


class JointSegClsNet(nn.Module):
    """Architecture 7A v3: Swin-T + SE + Attention Gates + deep
    supervision + multi-scale attention-pooling classifier."""
    def __init__(self, num_lesion_classes=2, num_dr_classes=5, pretrained=True):
        ...
        # Deep supervision: auxiliary heads at coarser decoder depths
        self.aux_seg_head_4 = nn.Conv2d(384, num_lesion_classes, kernel_size=1)  # after up4
        self.aux_seg_head_3 = nn.Conv2d(192, num_lesion_classes, kernel_size=1)  # after up3
    ...
    def set_encoder_trainable(self, trainable: bool):
        """Used for the freeze/unfreeze schedule during training."""
    ...
    def forward(self, x):
        ...
        cls_out = self.cls_head([e1, e2, e3, e4])
