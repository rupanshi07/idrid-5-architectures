...
def get_args():
    ...
    p.add_argument("--model_module", required=True, help="e.g. model_3a_simple_attention")
    p.add_argument("--tag", required=True, help="e.g. 3a_simple_attention")
...
def main():
    ...
    print(f"[{args.tag}] Using device: {device}")

    model_mod = importlib.import_module(args.model_module)
    JointSegClsNet = model_mod.JointSegClsNet
    ...
    try:
        model = JointSegClsNet(num_lesion_classes=len(LESIONS), pretrained=args.pretrained).to(device)
    except TypeError:
    ...
    out_dir = f"checkpoints_{args.tag}"
    ...
    print(f"\n[{args.tag}] Training complete. Best combined val score: {best_score:.4f}")
