...
from preprocessing_7e import get_roi_bbox, apply_preprocessing_pipeline, PRESETS
...
class IDRiDJointDataset(Dataset):
    def __init__(self, seg_img_dir=None, seg_mask_dir=None, cls_img_dir=None, cls_csv=None,
                 image_size=512, augment=False, zoom_crop_prob=0.25, preprocess_preset="P0"):
        self.image_size = image_size
        self.augment = augment
        self.zoom_crop_prob = zoom_crop_prob
        self.preprocess_config = PRESETS[preprocess_preset]
        self.samples = []
    ...
    def class_counts(self, num_classes=5):
        ...
        for sample in self.samples:
            if sample["has_cls"] and 0 <= sample["dr_grade"] < num_classes:
                counts[sample["dr_grade"]] += 1
        return counts
    ...
    def _get_sample(self, idx, augment, zoom_crop_prob=0.0):
        ...
        if sample["has_seg"]:
            raw_masks = {les: (Image.open(sample["mask_paths"][les]).convert("L") if sample["mask_paths"][les] else None) for les in LESIONS}
        else:
            raw_masks = {les: None for les in LESIONS}

        # ROI crop: SAME bounding box applied to image AND every mask,
        # per faculty's point #19 -- never apply intensity-only ops to masks
        if self.preprocess_config.get("roi_crop", False):
            left, top, right, bottom = get_roi_bbox(image)
            image = image.crop((left, top, right, bottom))
            raw_masks = {k: (v.crop((left, top, right, bottom)) if v is not None else None) for k, v in raw_masks.items()}

        # Deterministic intensity preprocessing (illumination + CLAHE + sharpen) -- image ONLY
        image = apply_preprocessing_pipeline(image, self.preprocess_config)
