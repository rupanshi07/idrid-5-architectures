"""
ARCHITECTURE 6, revised training script:
- Warm-starts encoder/decoder from Architecture 4's checkpoint (compatible
  shapes only) -- avoids retraining conv+transformer features from zero
- Disables focal-loss sharpening for classification (gamma=0, degenerates
  to plain weighted cross-entropy) -- less prone to mode collapse
- Lower cls_weight (1.5, down from 2.0) for more stable early training
"""
import argparse
import os
import torch
...
from model_6_custom_trans_attention import JointSegClsNet
...
def get_args():
    ...
    p.add_argument("--cls_focal_gamma", type=float, default=0.0)
    p.add_argument("--num_workers", type=int, default=4)
    p.add_argument("--out_dir", default="checkpoints_6_v2")
...
def load_compatible_weights(model, checkpoint_path, device):
    ...
    print(f"Loaded {len(compatible)} matching layers, reinitialized {len(skipped)}: {skipped[:6]}{'...' if len(skipped) > 6 else ''}")


def main():
    args = get_args()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"[Architecture 6 v2] Using device: {device}")
    ...
    model = JointSegClsNet(num_lesion_classes=len(LESIONS)).to(device)
    if args.resume_from:
        print(f"Warm-starting from {args.resume_from}")
        load_compatible_weights(model, args.resume_from, device)

    criterion = JointLoss(seg_weight=args.seg_weight, cls_weight=args.cls_weight,
                           seg_focal_alpha=seg_focal_alpha, cls_focal_gamma=args.cls_focal_gamma)
    ...
    for epoch in range(args.epochs):
        model.train()
        running = 0.0
        pbar = tqdm(train_loader, desc=f"[Arch6 v2] Epoch {epoch+1}/{args.epochs}")
        for batch in pbar:
            image = batch["image"].to(device); mask = batch["mask"].to(device)
    ...
    print(f"\n[Architecture 6 v2] Training complete. Best combined val score: {best_score:.4f}")
