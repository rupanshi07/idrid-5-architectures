...
def get_args():
    ...
    p.add_argument("--out_dir", default="checkpoints_1_simple_unet")
    return p.parse_args()
...
def main():
    ...
    for epoch in range(args.epochs):
        model.train()
        running = 0.0
        pbar = tqdm(train_loader, desc=f"[Simple U-Net] Epoch {epoch+1}/{args.epochs}")
        for batch in pbar:
    ...
    print(f"\n[Architecture 1] Training complete. Best combined val score: {best_score:.4f}")
