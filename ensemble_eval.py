...
from model_7a_swin_attention import JointSegClsNet as SwinNet
...
def main():
    ...
    print(f"[Ensemble: GAN + Swin-Attention] Using device: {device}")
    ...
    model_swin = SwinNet(num_lesion_classes=len(LESIONS), num_dr_classes=5, pretrained=False).to(device)
    model_swin.load_state_dict(torch.load("Results_7A_SwinAttention/model_best.pth", map_location=device))
    model_swin.eval()
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            seg2, cls2 = model_swin(image)

            # Average segmentation probabilities (not raw logits)
            seg_prob = (torch.sigmoid(seg1) + torch.sigmoid(seg2)) / 2.0
            pred_bin = (seg_prob > 0.5).float()

            # Average classification probabilities (softmax, not raw logits)
    ...
    if all_preds:
        ...
        print("\nConfusion matrix:")
        print(cm)
        plot_confusion_matrix(cm, [str(i) for i in range(5)], "ensemble_confusion_matrix.png",
                               "DR Grading -- Ensemble (GAN + Swin-Attention)")
